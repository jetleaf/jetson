...
import '../base.dart';
import '../serializers/dart_deserializer.dart';

/// {@template default_deserialization_context}
/// Default implementation of [DeserializationContext] in JetLeaf’s JSON subsystem.
///
/// The [DefaultDeserializationContext] acts as the **central deserialization registry**
/// for the [ObjectMapper]. It is responsible for locating, caching, and invoking
/// the correct [JsonDeserializer] for a given type during JSON-to-object conversion.
///
/// ### Responsibilities
/// - Manage user-defined and framework-provided deserializers.
/// - Resolve [JsonDeserializer] instances dynamically based on type compatibility.
/// - Provide fallback to generic [DartDeserializer] when no specific handler exists.
...
/// 4. Create a [DartDeserializer] if no match is found and cache it for future use.
///
/// ### Example
/// ```dart
/// final context = DefaultDeserializationContext(objectMapper, {});
...
final class DefaultDeserializationContext implements DeserializationContext {
  // ---------------------------------------------------------------------------
  // Fields
  // ---------------------------------------------------------------------------
  ...
  /// Cache of resolved [JsonDeserializer] instances, keyed by their [Class] type.
  ///
  ...
  Map<Class, JsonDeserializer> _configuredDeserializers = {};
  ...
  Map<Class, JsonDeserializer> _frameworkDeserializers = {};

  // ---------------------------------------------------------------------------
  // Constructor
  // ---------------------------------------------------------------------------

  /// Creates a new [DefaultDeserializationContext] associated with the given [ObjectMapper].
  ///
  ...
  DefaultDeserializationContext(this._objectMapper, this._deserializers) {
  ...
  JsonDeserializer? findDeserializerForType(Class type) {
    ...
    final dartDeserializer = DartDeserializer(type);
    _frameworkDeserializers[type] = dartDeserializer;
    return dartDeserializer;
  }

  /// Attempts to locate a [JsonDeserializer] for the specified [type]
  ...
  ///
  /// Returns the matching [JsonDeserializer], or `null` if no match is found.
  ///
  /// ### Example
  /// ```dart
  ...
  /// }
  /// ```
  JsonDeserializer? find(Class type, Map<Class, JsonDeserializer> deserializers) {
    if (deserializers.containsKey(type)) {
      return deserializers[type];
    }

    JsonDeserializer? deserializer = deserializers.values.find((deserializer) => deserializer.canDeserialize(type));
    if (deserializer != null) {
      return deserializer;
    }
